...
def init_chatbot():
    ...
    # --- Build the vector store from markdown files ---
    docs = []
    ...
    def answer_fn(message, history):
        """Accepts (message, history) and returns (updated_history)."""
        try:
            answer = retrieve_and_answer(message)
            history = history + [(message, answer)]  # ensure list of tuples
            return history
    ...
    with gr.Blocks() as demo:
        ...
        chatbot = gr.Chatbot(label="WorkFriend Conversation", type="tuple")
